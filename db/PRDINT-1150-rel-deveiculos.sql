...
AS SELECT veiculo.veigerenciadora1 AS __geroid1,
    ...
    (
        (CASE WHEN se0.atgdescricao IS NULL THEN ''::text ELSE se0.atgdescricao || ';'::text END  ||
         CASE WHEN se1.atgdescricao IS NULL THEN ''::text ELSE se1.atgdescricao || ';'::text END) ||
        CASE WHEN veiculo.veisatelital > 0 THEN 'Satelital;'::text ELSE ''::text END
    ) AS "Acessórios",
    (
        CASE WHEN s1.atgdescricao IS NULL THEN ''::text ELSE s1.atgdescricao || ';'::text END ||
        CASE WHEN s2.atgdescricao IS NULL THEN ''::text ELSE s2.atgdescricao || ';'::text END ||
        CASE WHEN s3.atgdescricao IS NULL THEN ''::text ELSE s3.atgdescricao || ';'::text END ||
        CASE WHEN s4.atgdescricao IS NULL THEN ''::text ELSE s4.atgdescricao || ';'::text END ||
        CASE WHEN s5.atgdescricao IS NULL THEN ''::text ELSE s5.atgdescricao || ';'::text END ||
        CASE WHEN s6.atgdescricao IS NULL THEN ''::text ELSE s6.atgdescricao || ';'::text END ||
        CASE WHEN s7.atgdescricao IS NULL THEN ''::text ELSE s7.atgdescricao || ';'::text END ||
        CASE WHEN s8.atgdescricao IS NULL THEN ''::text ELSE s8.atgdescricao || ';'::text END
    ) AS "Sensores",
    (
        CASE WHEN a1.atgdescricao IS NULL THEN ''::text ELSE a1.atgdescricao || ';'::text END ||
        CASE WHEN a2.atgdescricao IS NULL THEN ''::text ELSE a2.atgdescricao || ';'::text END ||
        CASE WHEN a3.atgdescricao IS NULL THEN ''::text ELSE a3.atgdescricao || ';'::text END ||
        CASE WHEN a4.atgdescricao IS NULL THEN ''::text ELSE a4.atgdescricao || ';'::text END ||
        CASE WHEN a5.atgdescricao IS NULL THEN ''::text ELSE a5.atgdescricao || ';'::text END ||
        CASE WHEN a6.atgdescricao IS NULL THEN ''::text ELSE a6.atgdescricao || ';'::text END ||
        CASE WHEN a7.atgdescricao IS NULL THEN ''::text ELSE a7.atgdescricao || ';'::text END ||
        CASE WHEN a8.atgdescricao IS NULL THEN ''::text ELSE a8.atgdescricao || ';'::text END
     ) AS "Atuadores"
   FROM veiculo
